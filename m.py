...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "You have not purchased yet purchase now from:- @rishabgupta01."
...
def clear_users_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "YOU DIDN'T ACCESS IF YOU WAT TO USE BUY FROM:- @RISHABGUPTA01 🙇."
    bot.reply_to(message, response)
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "ꜰʀᴇᴇ ᴋᴇ ᴅʜᴀʀᴍ ꜱʜᴀʟᴀ ʜᴀɪ ᴋʏᴀ ᴊᴏ ᴍᴜ ᴜᴛᴛʜᴀ ᴋᴀɪ ᴋʜɪ ʙʜɪ ɢᴜꜱ ʀʜᴀɪ ʜᴏ ʙᴜʏ ᴋʀᴏ ꜰʀᴇᴇ ᴍᴀɪ ᴋᴜᴄʜ ɴʜɪ ᴍɪʟᴛᴀ ʙᴜʏ:- @RISHABGUPTA01 ❄."
    bot.reply_to(message, response)
...
def show_recent_logs(message):
    ...
    if user_id in admin_id:
        ...
        response = "BUY NOW:- @RISHABGUPTA01❄."
        bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.🔥🔥\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP- User of @rishabgupta01"
    bot.reply_to(message, response)
...
Buy From :- @rishabgupta02
Official Channel :- https://t.me/amoryesports
'''
...
def welcome_start(message):
    ...
    response = f'''❄️AA GAYE BGMI KI MARNE, {user_name}! AAB HOGA MAUT KA NANGA NACH🫨🫨🫨. ᴛᴏ ɢᴇᴛ ᴀᴄᴄᴇss.
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, Brother BGMI MI MAA CHOD DE NA!!:
